...
// POST — Ghost Mode Spark/Pass vote + mutual connection check
export async function POST(req: NextRequest) {
  ...
  if (!myProfile || myProfile.age_track !== 'ghost') {
    return NextResponse.json({ error: 'Ghost Mode only' }, { status: 403 })
  }
  if (!targetProfile || targetProfile.age_track !== 'ghost') {
    return NextResponse.json({ error: 'Target user is not on Ghost Mode' }, { status: 400 })
  }

  const { error: voteErr } = await admin.from('sand_friend_votes').upsert({
    voter_id: user.id,
    target_id,
    vote,
  ...
  if (vote === 'spark') {
    ...
    if (reverseVote?.vote === 'spark') {
      const [u1, u2] = [user.id, target_id].sort()
      await admin.from('sand_connects').upsert({ user1_id: u1, user2_id: u2 }, { onConflict: 'user1_id,user2_id' })
      ...
      await admin.from('notifications').insert({
        ...
        body: 'Someone sparked with you back. Check your connects!',
      }).catch(() => {})
    }
  }

  return NextResponse.json({ success: true, connected, instagram_handle })
}
